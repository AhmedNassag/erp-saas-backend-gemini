...
export default {
  ...
  methods: {
    async loadItems() { try { const d = await this.api.getAll(); this.items = d.data || d } catch { notify({ text: 'Failed to load areas', type: 'error' }) } },
    ...
    async saveItem() {
      ...
      } catch (e) { notify({ text: e.response?.data?.message || 'Error saving area', type: 'error' }) } finally { this.saving = false }
    },
    async deleteItem(id) {
