...
import { branchesApi } from "@nexaerp/api"
...
async function fetchAll(page = 1) {
  loading.value = true
  try {
    const res = await branchesApi.list({ page, per_page: 15, search: search.value })
...
<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <h1 class="text-2xl font-black text-white">Branches</h1>
    </div>
