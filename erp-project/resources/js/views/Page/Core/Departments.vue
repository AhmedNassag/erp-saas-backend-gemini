  ...
  <div class="card card-flush">
    <div class="card-header pt-7">
      <h3 class="card-title align-items-start flex-column">
        ...
        <span class="text-gray-400 mt-1 fw-semibold fs-6">Manage system departments</span>
      </h3>
        ...
        <table class="table table-row-dashed table-row-gray-300 align-middle gs-0 gy-4">
          ...
          <tbody>
            ...
            <tr v-if="!items.length">
              <td colspan="3" class="text-center text-muted py-10">No departments found</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          <div class="modal-header">
            <h3 class="modal-title">{{ editingId ? 'Edit Department' : 'Add Department' }}</h3>
...
import Department from '../../../API/Modules/Core/Department/Department'
...
export default {
  ...
  data() {
    return {
      api: new Department('api/v1/core/department'),
      items: [],
      editingId: null,
      form: { name: '' },
  ...
  methods: {
    async loadItems() { try { const d = await this.api.getAll(); this.items = d.data || d } catch { notify({ text: 'Failed to load departments', type: 'error' }) } },
    ...
    async saveItem() {
      ...
      try {
        ...
        notify({ text: this.editingId ? 'Department updated' : 'Department created', type: 'success' })
        this.modal.hide(); this.loadItems()
      } catch (e) { notify({ text: e.response?.data?.message || 'Error saving department', type: 'error' }) } finally { this.saving = false }
    },
    async deleteItem(id) {
      if ((await Swal.fire({ title: 'Delete Department?', text: 'This action cannot be undone.', icon: 'warning', showCancelButton: true, confirmButtonText: 'Delete' })).isConfirmed) {
