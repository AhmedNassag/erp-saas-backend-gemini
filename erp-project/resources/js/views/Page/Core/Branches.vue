        ...
        <table class="table table-row-dashed table-row-gray-300 align-middle gs-0 gy-4">
          <thead>
            <tr class="fw-bold text-muted">
              ...
              <th class="min-w-100px">Commercial Registration</th>
              <th class="min-w-100px">Tax Card</th>
          ...
          <tbody>
            <tr v-for="item in items" :key="item.id">
              <td>{{ item.name }}</td>
              <td>{{ item.code }}</td>
              <td>{{ item.commercialRegistration }}</td>
              <td>{{ item.taxCard }}</td>
              <td>{{ item.mobile }}</td>
              <td>{{ item.address }}</td>
        ...
        <div class="modal-content">
          ...
          <form @submit.prevent="saveItem">
            <div class="modal-body">
              ...
              <div class="fv-row mb-7">
                <label class="required fs-6 fw-semibold mb-2">Tax Card</label>
                <input type="text" v-model="form.taxCard" class="form-control form-control-solid" />
              </div>
              <div class="fv-row mb-7">
