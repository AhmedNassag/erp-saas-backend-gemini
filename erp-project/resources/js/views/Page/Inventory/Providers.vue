  ...
  <div class="card card-flush">
    <div class="card-header pt-7">
      <h3 class="card-title align-items-start flex-column">
        <span class="card-label fw-bold text-dark">Providers</span>
        <span class="text-gray-400 mt-1 fw-semibold fs-6">Manage system providers</span>
      </h3>
      <div class="card-toolbar">
        <button type="button" class="btn btn-primary" @click="openForm()">
          <i class="ki-outline ki-plus fs-2"></i> Add Provider
        </button>
      </div>
    </div>
        ...
        <table class="table table-row-dashed table-row-gray-300 align-middle gs-0 gy-4">
          ...
          <tbody>
            ...
            <tr v-if="!items.length">
              <td colspan="9" class="text-center text-muted py-10">No providers found</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          <div class="modal-header">
            <h3 class="modal-title">{{ editingId ? 'Edit Provider' : 'Add Provider' }}</h3>
          ...
          <form @submit.prevent="saveItem">
            <div class="modal-body">
              ...
              <div class="fv-row mb-7">
                ...
                <input v-model="form.adresse" class="form-control form-control-solid" />
              </div>
            </div>
...
export default {
  ...
  data() {
    return {
      api: new Provider('api/v1/inventory/provider'),
      ...
      form: { name: '', code: '', phone: '', country_id: '', city_id: '', area_id: '', adresse: '' }, saving: false, modal: null, skipWatch: false,
    }
  },
  ...
  methods: {
    async loadItems() { try { const d = await this.api.getAll(); this.items = d.data || d } catch { notify({ text: 'Failed to load providers', type: 'error' }) } },
    ...
    openForm() {
      this.editingId = null
      this.form = { name: '', code: '', phone: '', country_id: '', city_id: '', area_id: '', adresse: '' }
    ...
    async editItem(item) {
      ...
      this.form.adresse = item.adresse || ''
    ...
    async saveItem() {
      ...
      } catch (e) { notify({ text: e.response?.data?.message || 'Error saving provider', type: 'error' }) } finally { this.saving = false }
    },
    async deleteItem(id) {
      if ((await Swal.fire({ title: 'Delete Provider?', text: 'This action cannot be undone.', icon: 'warning', showCancelButton: true, confirmButtonText: 'Delete' })).isConfirmed) {
