  ...
  <div class="card card-flush">
    <div class="card-header pt-7">
      <h3 class="card-title align-items-start flex-column">
        <span class="card-label fw-bold text-dark">Sale Return Details</span>
        <span class="text-gray-400 mt-1 fw-semibold fs-6">#{{ saleReturn.Ref }}</span>
      </h3>
      <div class="card-toolbar">
        ...
        <router-link to="/inventory/sale-returns" class="btn btn-light btn-sm">
    ...
    <div class="card-body pt-0">
      ...
      <template v-else-if="saleReturn.id">
        <div id="print_sale_return">
          <div class="row g-5 mb-8" id="sale-return-info-cards">
            <div class="col-md-3">
              <div class="card card-dashed card-flush bg-light rounded-3 p-5 h-100">
                ...
                <span class="fw-bold fs-4 text-gray-900">{{ saleReturn.date }}</span>
              </div>
            </div>
            <div class="col-md-3">
              <div class="card card-dashed card-flush bg-light rounded-3 p-5 h-100">
                ...
                <span class="fw-bold fs-4 text-gray-900">{{ saleReturn.client_name }}</span>
              </div>
            </div>
            <div class="col-md-3">
              <div class="card card-dashed card-flush bg-light rounded-3 p-5 h-100">
                ...
                <span class="fw-bold fs-4 text-gray-900">{{ saleReturn.warehouse_name }}</span>
              </div>
            </div>
            <div class="col-md-3">
              <div class="card card-dashed card-flush bg-light rounded-3 p-5 h-100">
                ...
                <span class="fw-bold fs-4">
                  <span class="badge fs-6 px-3 py-2 rounded-pill" :class="statusBadge(saleReturn.status)">
                    {{ saleReturn.status }}
                  </span>
                </span>
              </div>
            ...
            <div class="col-md-3">
              <div class="card card-dashed card-flush bg-light rounded-3 p-5 h-100">
                ...
                <span class="fw-bold fs-4">
                  <span class="badge fs-6 px-3 py-2 rounded-pill" :class="paymentBadge(saleReturn.payment_status)">
            ...
            <div class="col-md-3">
              <div class="card card-dashed card-flush bg-light rounded-3 p-5 h-100">
                ...
                <span class="fw-bold fs-4 text-gray-900">{{ saleReturn.user_name || '-' }}</span>
              </div>
            </div>
          </div>
          ...
          <div class="row justify-content-end mb-5">
            <div class="col-md-5">
              <div class="bg-light rounded-3 p-5">
                ...
                <div v-if="parseFloat(saleReturn.discount)" class="d-flex justify-content-between mb-2">
                  <span class="text-gray-600">Discount</span>
                  <span class="fw-semibold text-danger">- {{ saleReturn.discount }}</span>
                </div>
                <div v-if="parseFloat(saleReturn.shipping)" class="d-flex justify-content-between mb-2">
                  <span class="text-gray-600">Shipping</span>
                  <span class="fw-semibold">+ {{ saleReturn.shipping }}</span>
                </div>
                <div v-if="parseFloat(saleReturn.TaxNet)" class="d-flex justify-content-between mb-2">
                  <span class="text-gray-600">Tax ({{ saleReturn.tax_rate }}%)</span>
                  <span class="fw-semibold">+ {{ saleReturn.TaxNet }}</span>
                </div>
                <hr class="my-2" />
                <div class="d-flex justify-content-between mb-1">
                  <span class="fw-bold text-gray-900 fs-5">Grand Total</span>
                  <span class="fw-bold text-gray-900 fs-5">{{ saleReturn.GrandTotal }}</span>
                </div>
                <div class="d-flex justify-content-between mb-1">
                  <span class="text-gray-600">Paid</span>
                  <span class="fw-semibold text-success">{{ saleReturn.paid_amount }}</span>
                </div>
          ...
          <div v-if="saleReturn.notes" class="mb-5">
            <h5 class="fw-bold text-gray-900 mb-2">Notes</h5>
            <p class="text-gray-700 bg-light rounded-3 p-4">{{ saleReturn.notes }}</p>
          </div>
        </div>
      </template>
    </div>
...
export default {
  ...
  data() {
    return {
      api: new SaleReturn('api/v1/inventory/sale-return'),
      loading: true,
      saleReturn: {},
      detailRows: [],
      payments: [],
  ...
  computed: {
    ...
    due() {
      return (parseFloat(this.saleReturn.GrandTotal) || 0) - (parseFloat(this.saleReturn.paid_amount) || 0)
    },
    dueClass() {
      const d = this.due
  ...
  methods: {
    async loadDetail() {
      try {
        ...
        this.saleReturn = d.sale_return || d
        this.detailRows = d.details || []
        this.payments = this.saleReturn.payment_sale_returns || []
        this.loading = false
      } catch {
        notify({ text: 'Failed to load sale return details', type: 'error' })
        this.loading = false
      }
    },
    closeDropdowns() {
    ...
    printDetail(size) {
      this.closeDropdowns()
      const div = document.getElementById('print_sale_return')
      if (!div) return
      setTimeout(() => {
    ...
    async downloadPDF() {
      this.closeDropdowns()
      try {
        await this.api.downloadPdf(this.$route.params.id, `sale_return_${this.saleReturn.Ref}.pdf`)
      } catch {
...
<html>
<head><meta charset="utf-8"><title>Sale Return #${this.saleReturn.Ref}</title></head>
<body style="font-family:Arial,sans-serif;padding:20px">
  <h2>Sale Return Invoice</h2>
  <p><strong>Ref:</strong> ${this.saleReturn.Ref} | <strong>Date:</strong> ${this.saleReturn.date} | <strong>Client:</strong> ${this.saleReturn.client_name} | <strong>Warehouse:</strong> ${this.saleReturn.warehouse_name}</p>
  ...
  <p><strong>Grand Total:</strong> ${this.saleReturn.GrandTotal} | <strong>Paid:</strong> ${this.saleReturn.paid_amount} | <strong>Due:</strong> ${this.due}</p>
</body></html>`
      this.downloadFile(html, `sale_return_${this.saleReturn.Ref}.doc`, 'application/msword')
    },
    downloadCSV() {
      this.closeDropdowns()
      ...
      const rows = this.detailRows.map(d =>
        `"${this.saleReturn.date}","${this.saleReturn.Ref}","${this.saleReturn.client_name}","${this.saleReturn.warehouse_name}","${d.name || ''}","${d.code || ''}",${d.price || 0},${d.quantity || 0},"${d.unit || ''}",${d.total || 0},${this.saleReturn.GrandTotal},${this.saleReturn.paid_amount},${this.due},"${this.saleReturn.status || ''}"`
      ).join('\n')
      this.downloadFile('\uFEFF' + header + rows, `sale_return_${this.saleReturn.Ref}.csv`, 'text/csv;charset=utf-8')
    },
