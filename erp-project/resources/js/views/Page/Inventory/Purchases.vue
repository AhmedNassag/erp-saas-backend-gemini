  ...
  <div class="card card-flush">
    <div class="card-header pt-7">
      <h3 class="card-title align-items-start flex-column">
        <span class="card-label fw-bold text-dark">Purchases</span>
        <span class="text-gray-400 mt-1 fw-semibold fs-6">Manage purchase orders and payments</span>
      </h3>
      <div class="card-toolbar">
        <button type="button" class="btn btn-primary" @click="openForm()">
          <i class="ki-outline ki-plus fs-2"></i> Add Purchase
        </button>
      </div>
    </div>
        ...
        <table class="table table-row-dashed table-row-gray-300 align-middle gs-0 gy-4">
          ...
          <tbody>
            <tr v-for="item in items" :key="item.id">
              ...
              <td class="text-end">
                <router-link class="btn btn-icon btn-bg-light btn-active-color-info btn-sm me-1" :to="`/inventory/purchases/${item.id}/detail`">
            ...
            <tr v-if="!items.length">
              <td colspan="10" class="text-center text-muted py-10">No purchases found</td>
            </tr>
          </tbody>
        </table>
      </div>
        ...
        <div class="modal-content shadow-sm">
          <div class="modal-header border-bottom-0 pb-0">
            <div class="d-flex align-items-center">
              ...
              <div>
                <h3 class="modal-title fw-bold text-gray-900 mb-0">Purchase Details</h3>
        ...
        <div class="modal-content shadow-sm">
          <div class="modal-header border-bottom-0 pb-0">
            <div class="d-flex align-items-center">
              ...
              <div class="ms-3">
                <h3 class="modal-title fw-bold text-gray-900 mb-0">{{ editingId ? 'Edit Purchase' : 'Add Purchase' }}</h3>
                <span class="text-gray-500 fs-7">{{ editingId ? 'Update purchase order' : 'Create a new purchase order' }}</span>
              </div>
            </div>
            <button type="button" class="btn btn-sm btn-icon btn-bg-light btn-active-color-primary" data-bs-dismiss="modal">
              <i class="ki-outline ki-cross fs-2"></i>
...
import PaymentPurchase from '../../../API/Modules/Inventory/PaymentPurchase/PaymentPurchase'
...
export default {
  ...
  data() {
    return {
      purchaseApi: new Purchase('api/v1/inventory/purchase'),
      paymentApi: new PaymentPurchase('api/v1/inventory/payment-purchase'),
  ...
  methods: {
    async loadItems() {
      try { const d = await this.purchaseApi.getAll(); this.items = d.data || d || [] }
      catch { notify({ text: 'Failed to load purchases', type: 'error' }) }
    },
    statusBadge(s) {
    ...
    async editItem(id) {
      ...
      this.resetForm()
      try {
        const res = await this.purchaseApi.show(id)
        const d = res.data
        this.suppliers = d.providers || []
        this.warehouses = d.warehouses || []
        await this.loadUnits()
        this.form.provider_id = d.purchase?.provider_id || ''
        this.form.warehouse_id = d.purchase?.warehouse_id || ''
        this.form.date = d.purchase?.date || ''
        this.form.status = d.purchase?.status || 'pending'
        this.form.notes = d.purchase?.notes || ''
        this.form.tax_rate = d.purchase?.tax_rate || 0
        this.form.TaxNet = d.purchase?.TaxNet || 0
        this.form.discount = d.purchase?.discount || 0
        this.form.shipping = d.purchase?.shipping || 0
        this.form.GrandTotal = d.purchase?.GrandTotal || 0
        ...
        this.modal.show()
      } catch { notify({ text: 'Failed to load purchase', type: 'error' }) }
    },
    async showDetail(id) {
      try {
        const res = await this.purchaseApi.show(id)
        const d = res.data
        this.detailPurchase = d.purchase || {}
        this.detailRows = d.details || []
        this.payments = d.purchase?.payment_purchases || []
    ...
    async openPaymentsModal(item) {
      ...
      this.paymentHistory = []
      try {
        const res = await this.purchaseApi.show(item.id)
        this.paymentHistory = res.data?.purchase?.payment_purchases || []
        this.selectedPurchase = { ...this.selectedPurchase, ...res.data?.purchase }
      } catch { /* ignore */ }
    ...
    async saveItem() {
      ...
      try {
        ...
        if (this.editingId) await this.purchaseApi.update(this.editingId, fd); else await this.purchaseApi.insert(fd)
        notify({ text: this.editingId ? 'Purchase updated' : 'Purchase created', type: 'success' })
        this.modal.hide()
        this.saving = false
        this.loadItems()
      } catch (e) {
        const errors = e.response?.data?.errors
        if (errors && Object.keys(errors).length) { Object.entries(errors).forEach(([field, msgs]) => msgs.forEach(msg => notify({ text: field + ': ' + msg, type: 'error' }))) }
        else { notify({ text: e.response?.data?.message || 'Error saving purchase', type: 'error' }) }
        this.saving = false
      }
    },
    async deleteItem(id) {
      if ((await Swal.fire({ title: 'Delete Purchase?', text: 'This action cannot be undone.', icon: 'warning', showCancelButton: true, confirmButtonText: 'Delete' })).isConfirmed) {
        try { await this.purchaseApi.delete(id); this.loadItems() }
