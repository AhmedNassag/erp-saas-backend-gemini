  ...
  <div class="card card-flush">
    <div class="card-header pt-7">
      <h3 class="card-title align-items-start flex-column">
        <span class="card-label fw-bold text-dark">Sale Returns</span>
        <span class="text-gray-400 mt-1 fw-semibold fs-6">Manage sale return orders and payments</span>
      </h3>
      <div class="card-toolbar">
        <button type="button" class="btn btn-primary" @click="openForm()">
          <i class="ki-outline ki-plus fs-2"></i> Add Sale Return
        </button>
      </div>
    </div>
        ...
        <table class="table table-row-dashed table-row-gray-300 align-middle gs-0 gy-4">
          ...
          <tbody>
            <tr v-for="item in items" :key="item.id">
              ...
              <td class="text-end">
                <router-link class="btn btn-icon btn-bg-light btn-active-color-info btn-sm me-1" :to="`/inventory/sale-returns/${item.id}/detail`">
            ...
            <tr v-if="!items.length">
              <td colspan="10" class="text-center text-muted py-10">No sale returns found</td>
            </tr>
          </tbody>
        </table>
      </div>
        ...
        <div class="modal-content shadow-sm">
          <div class="modal-header border-bottom-0 pb-0">
            <div class="d-flex align-items-center">
              ...
              <div>
                <h3 class="modal-title fw-bold text-gray-900 mb-0">Sale Return Details</h3>
                <span class="text-gray-500 fs-7">Reference #{{ detailSaleReturn.Ref }}</span>
              </div>
            </div>
          ...
          <div class="modal-body pt-6">
            <div class="row g-5 mb-8">
              <div class="col-md-4">
                <div class="card card-dashed card-flush bg-light rounded-3 p-5 h-100">
                  ...
                  <span class="fw-bold fs-4 text-gray-900">{{ detailSaleReturn.date }}</span>
                </div>
              </div>
              <div class="col-md-4">
                <div class="card card-dashed card-flush bg-light rounded-3 p-5 h-100">
                  ...
                  <span class="fw-bold fs-4 text-gray-900">{{ detailSaleReturn.warehouse_name }}</span>
                </div>
              </div>
              <div class="col-md-4">
                <div class="card card-dashed card-flush bg-light rounded-3 p-5 h-100">
                  ...
                  <span class="fw-bold fs-4 text-gray-900">{{ detailSaleReturn.client_name }}</span>
                </div>
              </div>
              <div class="col-md-3">
                <div class="card card-dashed card-flush bg-light rounded-3 p-5 h-100">
                  ...
                  <span class="fw-bold fs-4 text-gray-900">{{ detailSaleReturn.user_name || '-' }}</span>
                </div>
              </div>
              <div v-if="detailSaleReturn.notes" class="col-12">
                <div class="card card-dashed card-flush bg-light-warning rounded-3 p-5">
                  <span class="fw-semibold fs-5 text-gray-800">{{ detailSaleReturn.notes }}</span>
                </div>
              </div>
            </div>
              ...
              <table class="table table-striped align-middle mb-0">
                <thead class="bg-gray-100">
                  <tr>
                    ...
                    <th class="fw-bold text-gray-700 text-end pe-6 min-w-70px">Price</th>
                ...
                <tbody>
                  <tr v-for="(d, idx) in detailRows" :key="idx">
                    ...
                    <td class="text-end pe-6">{{ d.price }}</td>
            ...
            <div v-if="detailSaleReturn.GrandTotal > 0" class="row g-5 mt-4 justify-content-end">
              <div class="col-md-5">
                <div class="bg-light rounded-3 p-4">
                  <div class="d-flex justify-content-between mb-2">
                    <span class="text-gray-600">Discount</span>
                    <span class="fw-bold">{{ detailSaleReturn.discount }}</span>
                  </div>
                  <div class="d-flex justify-content-between mb-2">
                    <span class="text-gray-600">Tax</span>
                    <span class="fw-bold">{{ detailSaleReturn.TaxNet }}</span>
                  </div>
                  <div class="d-flex justify-content-between mb-2">
                    <span class="text-gray-600">Shipping</span>
                    <span class="fw-bold">{{ detailSaleReturn.shipping }}</span>
                  </div>
                  <div class="separator my-2"></div>
                  <div class="d-flex justify-content-between mb-2">
                    <span class="fw-bold text-gray-800">Grand Total</span>
                    <span class="fw-bold fs-5 text-gray-900">{{ detailSaleReturn.GrandTotal }}</span>
                  </div>
                  <div class="d-flex justify-content-between">
                    <span class="text-gray-600">Paid</span>
                    <span class="fw-bold text-success">{{ detailSaleReturn.paid_amount }}</span>
                  </div>
                  <div class="d-flex justify-content-between">
                    <span class="text-gray-600">Due</span>
                    <span class="fw-bold" :class="dueClass(detailSaleReturn)">{{ detailSaleReturn.GrandTotal - detailSaleReturn.paid_amount }}</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div class="modal-content shadow-sm">
          <div class="modal-header border-bottom-0 pb-0">
            <div class="d-flex align-items-center">
              ...
              <div class="ms-3">
                <h3 class="modal-title fw-bold text-gray-900 mb-0">{{ editingId ? 'Edit Sale Return' : 'Add Sale Return' }}</h3>
                <span class="text-gray-500 fs-7">{{ editingId ? 'Update sale return order' : 'Create a new sale return order' }}</span>
              </div>
            </div>
            <button type="button" class="btn btn-sm btn-icon btn-bg-light btn-active-color-primary" data-bs-dismiss="modal">
              <i class="ki-outline ki-cross fs-2"></i>
          ...
          <form @submit.prevent="saveItem">
            <div class="modal-body pt-6">
              <div class="row g-5 mb-6">
                <div class="col-md-4">
                  <label class="required fs-6 fw-semibold mb-3 text-gray-700">Client</label>
                  <div class="position-relative">
                    <i class="ki-outline ki-user fs-2 position-absolute top-50 translate-middle-y ms-4 text-gray-500"></i>
                    <select v-model="form.client_id" class="form-select form-select-solid ps-12" required>
                ...
                <table class="table table-striped align-middle mb-0">
                  <thead class="bg-gray-100">
                    <tr>
                      ...
                      <th class="fw-bold text-gray-700 min-w-160px">Product</th>
                      <th class="fw-bold text-gray-700 text-center min-w-70px">Price</th>
                  ...
                  <tbody>
                    ...
                    <tr v-for="(d, i) in details" :key="d.detail_id">
                      ...
                      <td class="text-center">
                        <input type="number" min="0" step="0.01" v-model.number="d.price" class="form-control form-control-sm text-center" style="max-width:90px" @input="calcRowTotal(i)" />
                      </td>
                      <td class="text-center">
      ...
      <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content shadow-sm">
          <div class="modal-header border-bottom-0 pb-0">
            <div class="d-flex align-items-center">
              ...
              <div>
                ...
                <span class="text-gray-500 fs-7">{{ selectedSaleReturn?.Ref }}</span>
              </div>
            </div>
            <button type="button" class="btn btn-sm btn-icon btn-bg-light btn-active-color-primary" data-bs-dismiss="modal">
              <i class="ki-outline ki-cross fs-2"></i>
          ...
          <div class="modal-body pt-6">
            ...
            <div class="row g-3 mb-5">
              <div class="col-md-3">
                <div class="bg-light rounded-3 p-3 text-center">
                  ...
                  <span class="fw-bold fs-5 text-gray-900">{{ selectedSaleReturn?.GrandTotal }}</span>
                </div>
              </div>
              <div class="col-md-3">
                <div class="bg-light rounded-3 p-3 text-center">
                  <span class="text-gray-600 fs-7 fw-semibold d-block">Paid</span>
                  <span class="fw-bold fs-5 text-success">{{ selectedSaleReturn?.paid_amount }}</span>
                </div>
              </div>
              <div class="col-md-3">
                <div class="bg-light rounded-3 p-3 text-center">
                  <span class="text-gray-600 fs-7 fw-semibold d-block">Due</span>
                  <span class="fw-bold fs-5" :class="selectedSaleReturn && (selectedSaleReturn.GrandTotal - selectedSaleReturn.paid_amount) > 0 ? 'text-danger' : 'text-success'">{{ selectedSaleReturn?.GrandTotal - selectedSaleReturn?.paid_amount }}</span>
                </div>
              </div>
              <div class="col-md-3">
                <div class="bg-light rounded-3 p-3 text-center">
                  ...
                  <span class="fw-bold fs-5">
                    <span class="badge fs-7 px-3 py-2 rounded-pill" :class="paymentBadge(selectedSaleReturn?.payment_status)">{{ selectedSaleReturn?.payment_status }}</span>
                  </span>
                </div>
              </div>
            </div>
            ...
            <div v-if="selectedSaleReturn && (selectedSaleReturn.GrandTotal - selectedSaleReturn.paid_amount) > 0">
              ...
              <form @submit.prevent="savePayment">
                <div class="row g-5 mb-6">
                  ...
                  <div class="col-md-6">
                    ...
                    <div class="text-muted fs-7 mt-1">Due: {{ selectedSaleReturn?.GrandTotal - selectedSaleReturn?.paid_amount }}</div>
                  </div>
                  <div class="col-md-6">
...
import SaleReturn from '../../../API/Modules/Inventory/SaleReturn/SaleReturn'
import PaymentSaleReturn from '../../../API/Modules/Inventory/PaymentSaleReturn/PaymentSaleReturn'
...
export default {
  ...
  data() {
    return {
      saleReturnApi: new SaleReturn('api/v1/inventory/sale-return'),
      paymentApi: new PaymentSaleReturn('api/v1/inventory/payment-sale-return'),
      ...
      timer: null,
      form: { client_id: '', warehouse_id: '', date: new Date().toISOString().slice(0, 10), status: 'pending', tax_rate: 0, TaxNet: 0, discount: 0, shipping: 0, GrandTotal: 0, notes: '' },
      details: [],
      detailSaleReturn: {}, detailRows: [], payments: [],
  ...
  methods: {
    async loadItems() {
      try { const d = await this.saleReturnApi.getAll(); this.items = d.data || d || [] }
      catch { notify({ text: 'Failed to load sale returns', type: 'error' }) }
    },
    statusBadge(s) {
    ...
    resetForm() {
      this.form = { client_id: '', warehouse_id: '', date: new Date().toISOString().slice(0, 10), status: 'pending', tax_rate: 0, TaxNet: 0, discount: 0, shipping: 0, GrandTotal: 0, notes: '' }
      this.details = []
      this.searchInput = ''
      this.searchResults = []
      this.products = []
    ...
    async loadClients() {
      try {
        const d = await this.clientApi.getAll({ per_page: 1000 })
        this.clients = Array.isArray(d) ? d : (d.data || [])
      } catch { notify({ text: 'Failed to load clients', type: 'error' }) }
    },
    ...
    selectProduct(p) {
      ...
      this.details.push({
        ...
        price: p.price || 0,
        quantity: 1,
        sale_unit_id: p.unit_sale_id || (this.units.length ? this.units[0].id : null),
        unit_name: '',
        total: p.price || 0,
        TaxNet: 0,
        tax_method: '1',
    ...
    updateUnitName(i) {
      ...
      const unit = this.units.find(u => u.id === d.sale_unit_id)
    ...
    calcRowTotal(i) {
      const d = this.details[i]
      d.total = ((parseFloat(d.price) || 0) * (parseInt(d.quantity) || 0)).toFixed(2)
      this.calcTotals()
    },
    calcTotals() {
    ...
    verifiedForm() {
      ...
      if (!this.form.client_id) { notify({ text: 'Please select a client', type: 'warning' }); return false }
    ...
    async editItem(id) {
      ...
      this.resetForm()
      try {
        const res = await this.saleReturnApi.show(id)
        const d = res.data
        this.clients = d.clients || []
        this.warehouses = d.warehouses || []
        await this.loadUnits()
        this.form.client_id = d.sale_return?.client_id || ''
        this.form.warehouse_id = d.sale_return?.warehouse_id || ''
        this.form.date = d.sale_return?.date || ''
        this.form.status = d.sale_return?.status || 'pending'
        this.form.notes = d.sale_return?.notes || ''
        this.form.tax_rate = d.sale_return?.tax_rate || 0
        this.form.TaxNet = d.sale_return?.TaxNet || 0
        this.form.discount = d.sale_return?.discount || 0
        this.form.shipping = d.sale_return?.shipping || 0
        this.form.GrandTotal = d.sale_return?.GrandTotal || 0
        ...
        this.modal.show()
      } catch { notify({ text: 'Failed to load sale return', type: 'error' }) }
    },
    async showDetail(id) {
      try {
        const res = await this.saleReturnApi.show(id)
        const d = res.data
        this.detailSaleReturn = d.sale_return || {}
        this.detailRows = d.details || []
        this.payments = d.sale_return?.payment_sale_returns || []
        this.detailModal.show()
      } catch { notify({ text: 'Failed to load details', type: 'error' }) }
    },
    async openPaymentsModal(item) {
      this.selectedSaleReturn = item
      this.paymentForm = { date: new Date().toISOString().slice(0, 10), Reglement: 'Cash', montant: 0, notes: '' }
      this.paymentHistory = []
      try {
        const res = await this.saleReturnApi.show(item.id)
        this.paymentHistory = res.data?.sale_return?.payment_sale_returns || []
        this.selectedSaleReturn = { ...this.selectedSaleReturn, ...res.data?.sale_return }
      } catch { /* ignore */ }
    ...
    async savePayment() {
      ...
      try {
        await this.paymentApi.insert({
          sale_return_id: this.selectedSaleReturn.id,
    ...
    async saveEditPayment() {
      ...
      try {
        ...
        if (this.selectedSaleReturn) this.openPaymentsModal(this.selectedSaleReturn)
        this.loadItems()
      } catch (e) {
        const errors = e.response?.data?.errors
        if (errors && Object.keys(errors).length) { Object.entries(errors).forEach(([field, msgs]) => msgs.forEach(msg => notify({ text: field + ': ' + msg, type: 'error' }))) }
    ...
    async deletePayment(id) {
      if ((await Swal.fire({ title: 'Delete Payment?', text: 'This action cannot be undone.', icon: 'warning', showCancelButton: true, confirmButtonText: 'Delete' })).isConfirmed) {
        try {
          ...
          this.loadItems()
          if (this.selectedSaleReturn) this.openPaymentsModal(this.selectedSaleReturn)
          if (this.detailSaleReturn.id) this.showDetail(this.detailSaleReturn.id)
        }
    ...
    async saveItem() {
      ...
      try {
        ...
        this.details.forEach((d, i) => {
          ...
          fd.append(`details[${i}][price]`, d.price || 0)
          ...
          if (d.sale_unit_id) fd.append(`details[${i}][sale_unit_id]`, d.sale_unit_id)
          if (d.product_variant_id) fd.append(`details[${i}][product_variant_id]`, d.product_variant_id)
          if (d.id) fd.append(`details[${i}][id]`, d.id)
        })
        if (this.editingId) await this.saleReturnApi.update(this.editingId, fd); else await this.saleReturnApi.insert(fd)
        notify({ text: this.editingId ? 'Sale return updated' : 'Sale return created', type: 'success' })
        this.modal.hide()
        this.saving = false
        this.loadItems()
      } catch (e) {
        const errors = e.response?.data?.errors
        if (errors && Object.keys(errors).length) { Object.entries(errors).forEach(([field, msgs]) => msgs.forEach(msg => notify({ text: field + ': ' + msg, type: 'error' }))) }
        else { notify({ text: e.response?.data?.message || 'Error saving sale return', type: 'error' }) }
        this.saving = false
      }
    },
    async deleteItem(id) {
      if ((await Swal.fire({ title: 'Delete Sale Return?', text: 'This action cannot be undone.', icon: 'warning', showCancelButton: true, confirmButtonText: 'Delete' })).isConfirmed) {
        try { await this.saleReturnApi.delete(id); this.loadItems() }
