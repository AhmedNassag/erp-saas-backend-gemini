  ...
  <div class="card card-flush">
    <div class="card-header pt-7">
      <h3 class="card-title align-items-start flex-column">
        ...
        <span class="text-gray-400 mt-1 fw-semibold fs-6">Manage sales orders and payments</span>
      </h3>
      <div class="card-toolbar">
        <button type="button" class="btn btn-primary" @click="openForm()">
          <i class="ki-outline ki-plus fs-2"></i> Add Sale
        </button>
      </div>
    </div>
        ...
        <div class="modal-content shadow-sm">
          <div class="modal-header border-bottom-0 pb-0">
            <div class="d-flex align-items-center">
              ...
              <div>
                <h3 class="modal-title fw-bold text-gray-900 mb-0">Sale Details</h3>
                <span class="text-gray-500 fs-7">Reference #{{ detailSale.Ref }}</span>
              </div>
            </div>
          ...
          <div class="modal-body pt-6">
            <div class="row g-5 mb-8">
              <div class="col-md-4">
                <div class="card card-dashed card-flush bg-light rounded-3 p-5 h-100">
                  ...
                  <span class="fw-bold fs-4 text-gray-900">{{ detailSale.date }}</span>
                </div>
              </div>
              <div class="col-md-4">
                <div class="card card-dashed card-flush bg-light rounded-3 p-5 h-100">
                  ...
                  <span class="fw-bold fs-4 text-gray-900">{{ detailSale.warehouse_name }}</span>
                </div>
              </div>
              <div class="col-md-4">
                <div class="card card-dashed card-flush bg-light rounded-3 p-5 h-100">
                  ...
                  <span class="fw-bold fs-4 text-gray-900">{{ detailSale.client_name }}</span>
                </div>
              </div>
                <div class="col-md-3">
                <div class="card card-dashed card-flush bg-light rounded-3 p-5 h-100">
                  ...
                  <span class="fw-bold fs-4 text-gray-900">{{ detailSale.user_name || '-' }}</span>
                </div>
              </div>
            <div v-if="detailSale.notes" class="col-12">
                <div class="card card-dashed card-flush bg-light-warning rounded-3 p-5">
                  <span class="fw-semibold fs-5 text-gray-800">{{ detailSale.notes }}</span>
                </div>
              </div>
            </div>
            ...
            <div v-if="detailSale.GrandTotal > 0" class="row g-5 mt-4 justify-content-end">
              <div class="col-md-5">
                <div class="bg-light rounded-3 p-4">
                  <div class="d-flex justify-content-between mb-2">
                    <span class="text-gray-600">Discount</span>
                    <span class="fw-bold">{{ detailSale.discount }}</span>
                  </div>
                  <div class="d-flex justify-content-between mb-2">
                    <span class="text-gray-600">Tax</span>
                    <span class="fw-bold">{{ detailSale.TaxNet }}</span>
                  </div>
                  <div class="d-flex justify-content-between mb-2">
                    <span class="text-gray-600">Shipping</span>
                    <span class="fw-bold">{{ detailSale.shipping }}</span>
                  </div>
                  <div class="separator my-2"></div>
                  <div class="d-flex justify-content-between mb-2">
                    <span class="fw-bold text-gray-800">Grand Total</span>
                    <span class="fw-bold fs-5 text-gray-900">{{ detailSale.GrandTotal }}</span>
                  </div>
                  <div class="d-flex justify-content-between">
                    <span class="text-gray-600">Paid</span>
                    <span class="fw-bold text-success">{{ detailSale.paid_amount }}</span>
                  </div>
                  <div class="d-flex justify-content-between">
                    <span class="text-gray-600">Due</span>
                    <span class="fw-bold" :class="dueClass(detailSale)">{{ detailSale.GrandTotal - detailSale.paid_amount }}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="modal-header border-bottom-0 pb-0">
            <div class="d-flex align-items-center">
              <span class="symbol symbol-40 symbol-circle" :class="editingId ? 'bg-light-warning' : 'bg-light-success'">
                <i class="ki-outline fs-2" :class="editingId ? 'ki-pencil text-warning' : 'ki-cart text-success'"></i>
              </span>
              <div class="ms-3">
                <h3 class="modal-title fw-bold text-gray-900 mb-0">{{ editingId ? 'Edit Sale' : 'Add Sale' }}</h3>
                <span class="text-gray-500 fs-7">{{ editingId ? 'Update sale order' : 'Create a new sale order' }}</span>
              </div>
            </div>
            <button type="button" class="btn btn-sm btn-icon btn-bg-light btn-active-color-primary" data-bs-dismiss="modal">
              <i class="ki-outline ki-cross fs-2"></i>
                ...
                <div class="col-md-4">
                  ...
                  <div class="position-relative">
                    ...
                    <select v-model="form.client_id" class="form-select form-select-solid ps-12" required>
                      ...
                      <option v-for="s in clients" :key="s.id" :value="s.id">{{ s.name }}</option>
                    </select>
                  </div>
                </div>
                <div class="col-md-4">
                ...
                <table class="table table-striped align-middle mb-0">
                  <thead class="bg-gray-100">
                    <tr>
                      ...
                      <th class="fw-bold text-gray-700 text-center min-w-80px">Available</th>
                  ...
                  <tbody>
                    <tr v-if="!details.length">
                      <td colspan="9" class="text-center text-gray-500 py-8">Search and select products to add</td>
                    </tr>
                    <tr v-for="(d, i) in details" :key="d.detail_id">
                      ...
                      <td class="text-center">
                        <span class="badge badge-light-info fs-8 px-2 py-1">{{ getMaxQtyInSelectedUnit(d) }}</span>
                      </td>
                      <td class="text-center">
      ...
      <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content shadow-sm">
          <div class="modal-header border-bottom-0 pb-0">
            <div class="d-flex align-items-center">
              ...
              <div>
                ...
                <span class="text-gray-500 fs-7">{{ selectedSale?.Ref }}</span>
              </div>
            </div>
            <button type="button" class="btn btn-sm btn-icon btn-bg-light btn-active-color-primary" data-bs-dismiss="modal">
              <i class="ki-outline ki-cross fs-2"></i>
          ...
          <div class="modal-body pt-6">
            ...
            <div class="row g-3 mb-5">
              <div class="col-md-3">
                <div class="bg-light rounded-3 p-3 text-center">
                  ...
                  <span class="fw-bold fs-5 text-gray-900">{{ selectedSale?.GrandTotal }}</span>
                </div>
              </div>
              <div class="col-md-3">
                <div class="bg-light rounded-3 p-3 text-center">
                  <span class="text-gray-600 fs-7 fw-semibold d-block">Paid</span>
                  <span class="fw-bold fs-5 text-success">{{ selectedSale?.paid_amount }}</span>
                </div>
              </div>
              <div class="col-md-3">
                <div class="bg-light rounded-3 p-3 text-center">
                  <span class="text-gray-600 fs-7 fw-semibold d-block">Due</span>
                  <span class="fw-bold fs-5" :class="selectedSale && (selectedSale.GrandTotal - selectedSale.paid_amount) > 0 ? 'text-danger' : 'text-success'">{{ selectedSale?.GrandTotal - selectedSale?.paid_amount }}</span>
                </div>
              </div>
              <div class="col-md-3">
                <div class="bg-light rounded-3 p-3 text-center">
                  ...
                  <span class="fw-bold fs-5">
                    <span class="badge fs-7 px-3 py-2 rounded-pill" :class="paymentBadge(selectedSale?.payment_status)">{{ selectedSale?.payment_status }}</span>
                  </span>
                </div>
              </div>
            </div>
            ...
            <div v-if="selectedSale && (selectedSale.GrandTotal - selectedSale.paid_amount) > 0">
              ...
              <form @submit.prevent="savePayment">
                <div class="row g-5 mb-6">
                  ...
                  <div class="col-md-6">
                    ...
                    <div class="text-muted fs-7 mt-1">Due: {{ selectedSale?.GrandTotal - selectedSale?.paid_amount }}</div>
                  </div>
                  <div class="col-md-6">
...
import PaymentSale from '../../../API/Modules/Inventory/PaymentSale/PaymentSale'
...
export default {
  ...
  data() {
    return {
      ...
      paymentApi: new PaymentSale('api/v1/inventory/payment-sale'),
      ...
      details: [],
      detailSale: {}, detailRows: [], payments: [],
      selectedSale: null,
      paymentHistory: [],
  ...
  methods: {
    ...
    selectProduct(p) {
      ...
      if (p.qty <= 0) {
        notify({ text: `${p.name} has no available stock in this warehouse`, type: 'warning' }); return
      }
    ...
    getMaxQtyInSelectedUnit(d) {
      const unit = this.units.find(u => u.id === d.sale_unit_id)
      if (!unit || !d.current) return d.current || 0
      if (unit.operator == '/') {
        return Math.floor(d.current * unit.operator_value)
      }
      return Math.floor(d.current / unit.operator_value)
    },
    verifyQty(i) {
      ...
      const maxQty = this.getMaxQtyInSelectedUnit(d)
      if (d.quantity > maxQty) {
        d.quantity = maxQty
        notify({ text: `Quantity cannot exceed available stock (${maxQty})`, type: 'warning' })
      }
    },
    incrementQty(i) {
      const d = this.details[i]
      const maxQty = this.getMaxQtyInSelectedUnit(d)
      if ((d.quantity || 0) >= maxQty) {
        notify({ text: `Maximum available stock is ${maxQty}`, type: 'warning' }); return
      }
    ...
    async editItem(id) {
      ...
      this.resetForm()
      try {
        const res = await this.saleApi.show(id)
        const d = res.data
        this.clients = d.clients || []
        this.warehouses = d.warehouses || []
        await this.loadUnits()
        this.form.client_id = d.sale?.client_id || ''
        this.form.warehouse_id = d.sale?.warehouse_id || ''
        this.form.date = d.sale?.date || ''
        this.form.status = d.sale?.status || 'pending'
        this.form.notes = d.sale?.notes || ''
        this.form.tax_rate = d.sale?.tax_rate || 0
        this.form.TaxNet = d.sale?.TaxNet || 0
        this.form.discount = d.sale?.discount || 0
        this.form.shipping = d.sale?.shipping || 0
        this.form.GrandTotal = d.sale?.GrandTotal || 0
        ...
        this.details = (d.details || []).map((item, idx) => {
          ...
          return { ...item, detail_id: idx + 1, unit_name: item.unit || '', current: (prod?.qty || 0) + (item.quantity || 0) }
        })
        this.modal.show()
      } catch { notify({ text: 'Failed to load sale', type: 'error' }) }
    },
    async showDetail(id) {
      try {
        const res = await this.saleApi.show(id)
        const d = res.data
        this.detailSale = d.sale || {}
        this.detailRows = d.details || []
        this.payments = d.sale?.payment_sales || []
    ...
    async openPaymentsModal(item) {
      ...
      this.paymentHistory = []
      try {
        const res = await this.saleApi.show(item.id)
        this.paymentHistory = res.data?.sale?.payment_sales || []
        this.selectedSale = { ...this.selectedSale, ...res.data?.sale }
      } catch { /* ignore */ }
    ...
    async saveEditPayment() {
      ...
      try {
        ...
        if (this.selectedSale) this.openPaymentsModal(this.selectedSale)
        this.loadItems()
      } catch (e) {
        const errors = e.response?.data?.errors
        if (errors && Object.keys(errors).length) { Object.entries(errors).forEach(([field, msgs]) => msgs.forEach(msg => notify({ text: field + ': ' + msg, type: 'error' }))) }
    ...
    async deletePayment(id) {
      if ((await Swal.fire({ title: 'Delete Payment?', text: 'This action cannot be undone.', icon: 'warning', showCancelButton: true, confirmButtonText: 'Delete' })).isConfirmed) {
        try {
          ...
          this.loadItems()
          if (this.selectedSale) this.openPaymentsModal(this.selectedSale)
          if (this.detailSale.id) this.showDetail(this.detailSale.id)
        }
    ...
    async saveItem() {
      ...
      try {
        ...
        if (this.editingId) await this.saleApi.update(this.editingId, fd); else await this.saleApi.insert(fd)
        notify({ text: this.editingId ? 'Sale updated' : 'Sale created', type: 'success' })
        this.modal.hide()
        this.saving = false
        this.loadItems()
      } catch (e) {
        const errors = e.response?.data?.errors
        if (errors && Object.keys(errors).length) { Object.entries(errors).forEach(([field, msgs]) => msgs.forEach(msg => notify({ text: field + ': ' + msg, type: 'error' }))) }
        else { notify({ text: e.response?.data?.message || 'Error saving sale', type: 'error' }) }
        this.saving = false
      }
    },
    async deleteItem(id) {
      if ((await Swal.fire({ title: 'Delete Sale?', text: 'This action cannot be undone.', icon: 'warning', showCancelButton: true, confirmButtonText: 'Delete' })).isConfirmed) {
        try { await this.saleApi.delete(id); this.loadItems() }
