  ...
  <div class="card card-flush">
    <div class="card-header pt-7">
      <h3 class="card-title align-items-start flex-column">
        <span class="card-label fw-bold text-dark">Purchase Return Details</span>
      ...
      <div class="card-toolbar">
        ...
        <router-link to="/inventory/purchase-returns" class="btn btn-light btn-sm">
    ...
    <div class="card-body pt-0">
      ...
      <template v-else-if="purchase.id">
        <div id="print_purchase_return">
          <div class="row g-5 mb-8" id="purchase-return-info-cards">
            <div class="col-md-3">
...
export default {
  ...
  data() {
    return {
      api: new PurchaseReturn('api/v1/inventory/purchase-return'),
      loading: true,
      purchase: {},
      detailRows: [],
      payments: [],
  ...
  methods: {
    async loadDetail() {
      try {
        ...
        this.purchase = d.purchase_return || d
        this.detailRows = d.details || []
        this.payments = this.purchase.payment_purchase_returns || []
        this.loading = false
      } catch {
        notify({ text: 'Failed to load purchase return details', type: 'error' })
        this.loading = false
      }
    },
    closeDropdowns() {
    ...
    printDetail(size) {
      this.closeDropdowns()
      const div = document.getElementById('print_purchase_return')
      if (!div) return
      setTimeout(() => {
    ...
    async downloadPDF() {
      this.closeDropdowns()
      try {
        await this.api.downloadPdf(this.$route.params.id, `purchase_return_${this.purchase.Ref}.pdf`)
      } catch {
...
<html>
<head><meta charset="utf-8"><title>Purchase Return #${this.purchase.Ref}</title></head>
<body style="font-family:Arial,sans-serif;padding:20px">
  ...
  <p><strong>Grand Total:</strong> ${this.purchase.GrandTotal} | <strong>Paid:</strong> ${this.purchase.paid_amount} | <strong>Due:</strong> ${this.due}</p>
      ...
      this.downloadFile(html, `purchase_return_${this.purchase.Ref}.doc`, 'application/msword')
    },
    downloadCSV() {
      this.closeDropdowns()
      ...
      this.downloadFile('\uFEFF' + header + rows, `purchase_return_${this.purchase.Ref}.csv`, 'text/csv;charset=utf-8')
    },
