  ...
  <div class="card card-flush">
    <div class="card-header pt-7">
      <h3 class="card-title align-items-start flex-column">
        <span class="card-label fw-bold text-dark">Currencies</span>
        <span class="text-gray-400 mt-1 fw-semibold fs-6">Manage system currencies</span>
      </h3>
        ...
        <table class="table table-row-dashed table-row-gray-300 align-middle gs-0 gy-4">
          <thead>
            <tr class="fw-bold text-muted">
              ...
              <th class="min-w-100px">Symbol</th>
          ...
          <tbody>
            ...
            <tr v-if="!items.length">
              <td colspan="5" class="text-center text-muted py-10">No currencies found</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          <div class="modal-header">
            <h3 class="modal-title">{{ editingId ? 'Edit Currency' : 'Add Currency' }}</h3>
          ...
          <form @submit.prevent="saveItem">
            <div class="modal-body">
              ...
              <div class="fv-row mb-7">
                <label class="required fs-6 fw-semibold mb-2">Symbol</label>
                <input type="text" v-model="form.symbol" class="form-control form-control-solid" required />
              </div>
            </div>
...
export default {
  ...
  data() {
    return {
      api: new Currency('api/v1/inventory/currency'),
      items: [], editingId: null,
      form: { name: '', code: '', symbol: '' }, saving: false, modal: null,
    }
  },
  mounted() { this.loadItems(); this.modal = new Modal(this.$refs.modalEl) },
  methods: {
    async loadItems() { try { const d = await this.api.getAll(); this.items = d.data || d } catch { notify({ text: 'Failed to load currencies', type: 'error' }) } },
    openForm() {
      this.editingId = null
    ...
    async saveItem() {
      ...
      try {
        ...
        else { notify({ text: e.response?.data?.message || 'Error saving currency', type: 'error' }) }
        this.saving = false
      }
    },
    async deleteItem(id) {
      if ((await Swal.fire({ title: 'Delete Currency?', text: 'This action cannot be undone.', icon: 'warning', showCancelButton: true, confirmButtonText: 'Delete' })).isConfirmed) {
