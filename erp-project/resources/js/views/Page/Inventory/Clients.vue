  ...
  <div class="card card-flush">
    <div class="card-header pt-7">
      <h3 class="card-title align-items-start flex-column">
        <span class="card-label fw-bold text-dark">Clients</span>
        <span class="text-gray-400 mt-1 fw-semibold fs-6">Manage system clients</span>
      </h3>
      <div class="card-toolbar">
        <button type="button" class="btn btn-primary" @click="openForm()">
          <i class="ki-outline ki-plus fs-2"></i> Add Client
        </button>
      </div>
    </div>
        ...
        <table class="table table-row-dashed table-row-gray-300 align-middle gs-0 gy-4">
          ...
          <tbody>
            ...
            <tr v-if="!items.length">
              <td colspan="9" class="text-center text-muted py-10">No clients found</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          <div class="modal-header">
            <h3 class="modal-title">{{ editingId ? 'Edit Client' : 'Add Client' }}</h3>
...
export default {
  ...
  data() {
    return {
      api: new Client('api/v1/inventory/client'),
  ...
  methods: {
    async loadItems() { try { const d = await this.api.getAll(); this.items = d.data || d } catch { notify({ text: 'Failed to load clients', type: 'error' }) } },
    ...
    async saveItem() {
      ...
      try {
        ...
        notify({ text: this.editingId ? 'Client updated' : 'Client created', type: 'success' })
        this.modal.hide(); this.loadItems()
      } catch (e) { notify({ text: e.response?.data?.message || 'Error saving client', type: 'error' }) } finally { this.saving = false }
    },
    async deleteItem(id) {
      if ((await Swal.fire({ title: 'Delete Client?', text: 'This action cannot be undone.', icon: 'warning', showCancelButton: true, confirmButtonText: 'Delete' })).isConfirmed) {
