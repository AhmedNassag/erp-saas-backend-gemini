...
class UpdateRequest extends FormRequest
{
    ...
    public function withValidator($validator)
    {
        ...
        $validator->after(function ($validator) {
            ...
            $id = $this->route('id');
            $oldDetails = $id ? TransferDetail::where('transfer_id', $id)->get() : collect();
            ...
            foreach ($this->input('details', []) as $i => $detail) {
                ...
                // Add back old quantities that will be reverted before applying new ones
                $oldQty = 0;
                foreach ($oldDetails as $od) {
                    if ($od->product_id == $detail['product_id'] &&
                        $od->product_variant_id == ($detail['product_variant_id'] ?? null)) {
                        $unit = Unit::find($od->purchase_unit_id);
                        $odQty = $od->quantity;
                        if ($unit) {
                            $odQty = $unit->operator == '/'
                                ? $od->quantity / $unit->operator_value
                                : $od->quantity * $unit->operator_value;
                        }
                        $oldQty += $odQty;
                    }
                ...
                $available = $stockQty + $oldQty;

                if ($transferQty > $available) {
                    $validator->errors()->add(
                        "details.{$i}.quantity",
                        "الكمية المطلوبة ({$qty}) تتجاوز المخزون المتاح ({$available})"
                    );
                }
            }
        });
